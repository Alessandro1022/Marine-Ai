  ...
  return (
    <div className="space-y-6 p-4 pb-28">
      ...
      {weather && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-sonar/10 border border-sonar/20 rounded-xl p-4 space-y-2">
            ...
            <p className="text-3xl font-bold text-mist">
              {Math.round(weather.temperature_c)}°
            </p>
            <p className="text-xs text-mist/50">C</p>
          </div>
          ...
          <div className="bg-sonar/10 border border-sonar/20 rounded-xl p-4 space-y-2">
            ...
            <p className="text-3xl font-bold text-mist">
              {Math.round(weather.wind_speed_ms)}
            </p>
          ...
          {weather.wave_height_m && (
            <div className="bg-sonar/10 border border-sonar/20 rounded-xl p-4 space-y-2">
              ...
              <p className="text-3xl font-bold text-mist">
                {Math.round(weather.wave_height_m * 10) / 10}
              </p>
        ...
        {trips.length > 0 && (
          <div className="bg-sonar/10 border border-sonar/20 rounded-xl p-4">
            ...
            <p className="text-xs text-mist/60">
              {Math.round(totalDistance)} NM totalt
            </p>
          </div>
        )}
      </div>
      ...
      {forecast && forecast.hourly && forecast.hourly.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-mist/80 mb-3 uppercase tracking-wider">
            Väderprognos nästa 12 timmar
          </h2>
          <div className="grid grid-cols-3 gap-2">
            {forecast.hourly.slice(0, 6).map((hour: any, idx: number) => (
              <div
                key={idx}
                className="bg-sonar/10 border border-sonar/20 rounded-lg p-3 text-center text-xs"
              >
                <p className="text-mist/60 mb-1">
                  {new Date(hour.time).toLocaleTimeString("sv-SE", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </p>
                <p className="text-lg font-bold text-sonar">
                  {Math.round(hour.temperature_c)}°
                </p>
                {hour.wind_speed_ms && (
                  <p className="text-mist/60 mt-1">
                    💨 {Math.round(hour.wind_speed_ms)} m/s
                  </p>
                )}
              </div>
            ))}
      ...
      <div className="grid grid-cols-2 gap-3 pt-2">
        <a
          href="/map"
          className="block p-4 bg-sonar/20 hover:bg-sonar/30 text-sonar border border-sonar/30 rounded-xl font-semibold text-center transition"
        >
          🗺️<br />
        ...
        >
          ⚓<br />
          <span className="text-sm">Resor</span>
        </a>
        <a
          href="/boats"
          className="block p-4 bg-sonar/20 hover:bg-sonar/30 text-sonar border border-sonar/30 rounded-xl font-semibold text-center transition"
        >
          ⛵<br />
          <span className="text-sm">Båtar</span>
        </a>
      </div>
    </div>
  );
